...
class AutoUpdater(private val mContext: Context) {
    ...
    private val downApkWork = Runnable {
        try {
            ...
            conn.setRequestProperty("User-Agent", "Doraemon-App-Update-Checker")
    ...
    fun checkUpdate(
        ...
        Thread(Runnable {
            ...
            if (!config.isNullOrEmpty()) {
                try {
                    val releaseJson = JSONObject.parseObject(config)
                    val tagName = releaseJson.getString("tag_name") ?: ""
                    remoteVersion = tagName.replace("v", "").replace("V", "")
            ...
            if (apkUrl.isNotEmpty() && checkIfValidVersion(remoteVersion)) {
    ...
    private fun doGet(httpUrl: String?): String? {
        ...
        try {
            ...
            connection.setRequestProperty("User-Agent", "Doraemon-App-Update-Checker")
